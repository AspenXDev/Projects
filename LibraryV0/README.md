...
- To fulfill NTUC Learning Hub's Skillsfuture Career Transition Program (SCTP) in Fullstack Development's Capstone Project module.
- Showcase knowledge and proficiency in the following: - Java and Java Spring Boot proficiency in MVC architecture - JavaScript and ReactJS - Database awareness of both SQL and NoSQL solutions and appropriateness - Software design and architecture - Linux
- Demonstrate 15+ years' project management experience and engineering background as transferrable skills for problem solving in software too.
...
Matrix of pages/components rendered based on role

| Role | Pages/Components Access |

| Member | Search, Book Detail, My Profile, My Loans |

| Staff | Member functions + Borrow/Return Interface |

| Librarian | Staff functions + Cataloging Tools |

| Admin | All access; Sys config |

#### Access Control (ACL-style enforcement)?

- All access is role-based (RBAC) (page blocking with routes)
...
4. If valid, returns an HTTP 200 and user info/token?
...
| Activity/Deliverable                      | Product Owner/Manager/SCRUM Master (Victor) | Developer (Victor) | QA/Tester (Victor) | **IT Operations** |
| ----------------------------------------- | ------------------------------------------- | ------------------ | ------------------ | ----------------- |
...
- **Unit Testing (UT):** This is the responsibility of the dev. As code is written, create unit tests for individual functions and components to ensure they work as expected.
...
- Current version v0 is designed for physical community center's on-prem intranet-use only. Future versions to consider higher security for internet access (eg. row level security with PostGreSQL).
- Data (users+books+logs) Migration script to be developed in future versions if use case arises. Dummy data has been used for now. CSV imports/exports to be considered too.
